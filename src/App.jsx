...
// ─── OBSERVE ENTRY — lightweight 2-tap shell ────────────────────────────────
function ObserveEntry({ onRoute, onClose }) {
  ...
  const optBtn = (selected) => ({
    ...
    border: `0.5px solid ${selected ? "var(--amber)" : "var(--border)"}`,
    borderRadius: "var(--r)", color: selected ? "var(--amber)" : "var(--text)",
    fontSize: 14, cursor: "pointer", fontFamily: "'DM Sans', sans-serif",
    textAlign: "left", transition: "all 0.15s", WebkitTapHighlightColor: "transparent"
  });

  const route = (needState) => {
    const regType = (() => { try { return localStorage.getItem("stillform_regulation_type") || "balanced"; } catch { return "balanced"; } })();
    const bioFilter = (() => { try { return localStorage.getItem("stillform_bio_filter") || ""; } catch { return ""; } })();
    ...
    if (needState === "understand") {
      ...
      if (regType === "thought-first") { onRoute("reframe"); return; }
      if (regType === "body-first") { onRoute("scan"); return; }
      onRoute("metacognition"); return;
    }
    if (needState === "catch") { onRoute("metacognition"); return; }
  };
    ...
    <div style={{ padding: "4px 0" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {[
          ...
          <button key={opt.id} onClick={() => route(opt.id)} style={optBtn(false)}>
            <span style={{ fontWeight: 500 }}>{opt.label}</span>
            <span style={{ fontSize: 12, color: "var(--text-dim)", marginLeft: 8 }}>{opt.sub}</span>
          </button>
        ))}
          ...
          return (
            <section style={{ maxWidth: 420, margin: "0 auto", padding: "40px 24px 80px", position: "relative", zIndex: 1 }}>
              ...
              <div style={{ marginBottom: 48, animation: "entrain60glow 1s ease-in-out infinite" }}>
                ...
                {/* PRIMARY — Observe and Choose entry shell */}
                {showObserveEntry ? (
                  <ObserveEntry
                    ...
                    onRoute={(target) => {
                      ...
                      else if (target === "metacognition") { setActiveTool({ ...TOOLS.find(t => t.id === "metacognition") }); setScreen("tool"); }
                    }}
                  />
                ) : (
                  <>
