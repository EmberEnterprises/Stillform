          ...
          const FAQ_ITEMS = [
{
              ...
              },
              {
                ...
                a: "The chip you select is a structural signal that adjusts how Stillform meets you in that moment. The act of selecting one is itself part of the work, and the architecture responds to it on multiple layers. Naming the state regulates it. Affect labeling — putting an emotion into a single word — measurably reduces activity in the amygdala (the brain\'s threat detector) and increases activity in the right ventrolateral prefrontal cortex (the regulatory system). The shift happens in seconds, before any tool runs. Tapping the chip is not just an input. It is the first intervention. The chip routes you to the response that fits. Different states respond to different interventions. High activation (Anxious, Angry) responds to slowing down and naming what is present. Cognitive looping (Stuck) responds to interruption rather than processing. Activated forward states (Excited, Focused) respond to maintained momentum, not de-activation. Disconnection from the body (Distant) responds to physical input, not language — so it routes you to Body Scan instead of Reframe entirely. The chip records the moment. It is logged automatically to your Signal Log on the first message of a session, joins your bio-filter as paired physical-and-emotional context, and is recorded as the pre-state for that session. Over time, your pre/post chip selections build a longitudinal record of how your states shift through the practice — visible to you in My Progress, never accessible to anyone else. The chip persists until something legitimately changes it. State does not reset on a clock. The chip you select holds across sessions, refreshes, and screen changes. It clears when you explicitly change it, when you complete What Shifted at the end of a session, or when you log a new morning check-in. The system does not silently overwrite your last self-report. The chip protects you at high activation. When you arrive at Anxious, Angry, or Mixed, certain elements that could read as comparison or pressure are suppressed during the session — the goal is to meet you where you are, not to remind you of a more regulated version of yourself. Once the immediate intensity passes, Stillform offers Self Mode as a deeper option. Repeated specific labeling builds the skill. The ability to distinguish between specific states, rather than broad categories, is consistently associated with better regulation and more adaptive responses over time. Every chip selection is one rep of that practice. The chip is a snapshot of where you are right now. It is not a label on you, and the choice is reversible at any point in the session."
              },
              {
