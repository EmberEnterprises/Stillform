...
### CONNECTIVE-TISSUE AUDIT (May 28/29 2026) — what's built-as-capability but NOT wired to the user
Done in response to Arlin's "is everything intertwined?" / "are there gaps?" Verified by reading the actual code, not the summary. The PIECES are largely built; some cross-session connective wiring is not hooked up.
- **In-session AI intertwining — mostly WIRED.** `reframeApi.sendReframeMessage` sends biasProfile + triggerProfile + contextProfile + priorSessions + todayThread + feelState to reframe.js. **Dark:** the client hardcodes `bioFilter: null` and `signalProfile: null` — so the elaborate bio-filter cross-referencing logic in reframe.js never fires. Cause: the bio-filter + signal-map CAPTURE surfaces are the dropped Tier-B friction layers (WAVE 3 — await Arlin's friction-vs-value call per layer). Not a bug; a parked product decision.
- **Mirror strip — DARK.** `setMirrorObservation` is never called anywhere. SmartScreen (the live home, mounted inside Home.jsx — NOT Today.jsx, which is retired) renders the Mirror only when `getMirrorObservation()` is non-null, so the Mirror section is permanently hidden. The Mirror is "the most concentrated expression of the User-AI relationship" — and it never lights up.
- **AI Mediation pipeline (PROPOSE_UPDATE) — DARK client-side.** The backend mode exists in reframe.js (proposes updates to Trigger Profile / Habit Anchors / Capacity Baseline from evidence) but NOTHING on the client calls it. Same pipeline is what would generate the Mirror observation.
- **Live on the home (confirmed working):** Thread (`getTodayThread`), Active prompt (`getActivePromptAsync` + fallback), Trajectory (`getTrajectoryStats`, mechanical). 3 of 4 home sections live; only Mirror dark.
- **The 4 diagnostic mirror screens** (Capacities / Risk / PredictionError / WhatYouBetOn) — reachable/routed in AppV2. Wired.
- **WIRING SCOPE (next build — needs Arlin's calls, do NOT blind-build):** wire a client trigger that runs the AI Mediation pipeline → generates a spec-compliant Mirror observation (locked voice: "Seven variants of criticism spiraling named over three weeks. Body always: shoulders, jaw" + Zero-Fabrication + User-AI Principle A reflect-not-interpret) → `setMirrorObservation`; SmartScreen already renders it. **FORKS (founder-domain):** (1) trigger point + frequency = token cost = money call; (2) ship default-ON or default-OFF-with-knob (mirrors the gating-knob pattern); (3) whether to also surface the artifact-update proposals to the user, and where. **Tier-B bio-filter/signal capture = separate friction call.** Lock in Master Todo Phase 5 AI-Mediation scope when Arlin scopes the build.

### VERIFIED CODE AUDIT — true status vs. the docs (May 29 2026, at Arlin's request)
Audited the "outside-the-phases" items against the ACTUAL live frontend (`src/v2/`) + `netlify/functions/`, NOT the docs. Confirms Arlin's instinct: several items the docs marked "shipped/resolved" were shipped in the OLD frontend (`src/App.jsx`, deleted `237167b`, May 17) and have not been rebuilt yet in the ground-up front-end rebuild — that is what `src/v2/` IS. These are REBUILD BACKLOG, not losses; the old front end was deliberately deleted May 17 to rebuild it. The Master Todo's "RESOLVED" tags on any `App.jsx`-referenced item are NOT TRUE for the live build. Treat THIS as the true status; re-baseline the non-phase items against it.
...
**Doc debt:** testing checklist rewrite, COPY_LOCKS reconciliation, a real targeted preflight script.
- **Sweep extended to Phase 6/7 (May 28, parity):** framing re-verified by grep (the regulate/anxious hits are legit science-citations / "Deep Regulate" tool name / "anxious" chip — not positioning); F·S·F recorded for PCE.1-3 + 6.2-6.5 + Phase 7; API contracts verified — pre-event-brief `{hardware,risks,moves,recovery}` ✓, rehearsal `{exchanges:[{they,you}],note,crisis}` ✓, scripts `{script,tone,note}` + crisis derived client-side from `tone==="crisis-redirect"` (consistent end-to-end) ✓; no leftover debug. Today's entire body of work now carries the complete protocol, not a partial pass.
