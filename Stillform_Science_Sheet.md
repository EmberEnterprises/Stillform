...
> **⚠️ Framing reference (added May 12, 2026):** STILLFORM_FRAMING_LAW.md is the supreme reference for what Stillform IS. The Architecture section below is aligned with it. Where individual feature sections in this doc use older framing language (e.g., "the entire product purpose," "regulation app" phrasing), the framing law supersedes. Per-feature science citations (Mehling, Critchley, Russell, Lehrer, etc.) remain valid as feature-level evidence — they are NOT the product-defining science spine. The product-defining science spine lives in STILLFORM_FRAMING_LAW.md and includes: metacognition (Flavell, Schraw & Moshman, Veenman, Frontiers 2026), adaptive vs maladaptive metacognition (Wells 2009, Hitchcock 2024), emotional granularity (Barrett 2001, Kashdan et al. 2015, Hoemann 2021), EMI methodology (Myin-Germeys 2016/2018), neuroplasticity (Davidson, Lazar 2005, Calderone 2024), self-mastery (Bandura 1977, sense of mastery), decision-making (Kahneman 2011), affect labeling as ONE supporting mechanism (Lieberman 2007), cognitive reappraisal (Ochsner & Gross 2005), supporting regulation/somatic science (Porges, Gerritsen & Band 2018, Khalsa 2018, Lehrer 2020).
...
**Stillform is a metacognition practice.** Users build the concept library the brain uses to perceive its own internal states. The mechanism is concept-formation through repeated structured analytical work. As the library deepens, pattern recognition operates at increasing speed — eventually fast enough to feel like direct perception. Composure is one felt outcome among several that signal the practice is working. Composure is NOT the product. See STILLFORM_FRAMING_LAW.md for the supreme reference.

The four pillars below organize the supporting feature-level science. They are NOT the product-defining science spine (that's in the framing law).

**Metacognition (the mechanism).** How the user names, analyzes, observes their own processing with increasing granularity over time. Flavell (1979); Schraw & Moshman (1995); Veenman et al. (2006); Frontiers in Psychology (2026) review synthesizing metacognition + neuroplasticity. Wells (2009) MCT on detached mindfulness — useful but Stillform's practice is more analytical than MCT alone. Hitchcock et al. (2024) meta-control account defines the rumination failure mode the practice must avoid. Brewer et al. (2011) on Default Mode Network suppression during single-pointed attention. Features: Self Mode (5-step practice), Reframe (bounded metacognitive sessions), Pattern Recognition / Insight Card (concept-applying surface), What Shifted (closure ritual), Skip tracking (avoidance pattern data), bio-filter (predictive interoception entry point), calibration (routing tendency identification).

**Concept-building / Emotional awareness (input).** Building the concept library the brain uses to perceive itself. Barrett (2017) constructed emotion theory — the brain perceives internal states by constructing them using concepts; richer concept library = more granular perception. Barrett et al. (2001) on emotional granularity (the trainable skill). Kashdan, Barrett, McKnight (2015) review — higher granularity correlates with less maladaptive regulation, less neural reactivity to rejection, less severe anxiety/depression. Hoemann et al. (2021) — experience sampling itself increases granularity; this directly grounds Stillform's pulse log + chip system. Suvak et al. (2011) on granularity in BPD population. Mehling et al. (2012) on interoceptive awareness; Critchley & Garfinkel (2017) on interoception and emotion. Lieberman et al. (2007), Nook et al. (2021) on affect labeling as ONE supporting mechanism. Vine, Bernstein & Nolen-Hoeksema (2019) on free-generated labels. Seth (2013), Barrett & Simmons (2015) on interoceptive predictive coding. Features: feel chips (post-only after Apr 28 fix), What Shifted free text, bio-filter, granularity tracking, pulse log.

**Microbiases (outward gaze, downstream).** What concept-rich perception produces when applied to social interaction. Genzer, Rubin, Sened, Rafaeli, Ochsner, Cohen, & Perry (2025, *Nature Communications*); Ross (1977) on fundamental attribution error; Hatfield, Cacioppo & Rapson (1993) on emotional contagion. Features: AI microbias awareness, intersection-flagging when bio-filter shows depletion plus negative read of others.

**Neuroplasticity (the glue — practice compounds).** What makes the practice deepen across sessions instead of resetting. Davidson (Center for Healthy Minds) on contemplative neuroplasticity. Lazar et al. (2005) on cortical thickening from meditation. Calderone et al. (2024) — meditation produces measurable cortical thickness and connectivity changes (more recent than Lazar). BDNF as neuroplasticity mediator (Frontiers 2026 review synthesis). Myin-Germeys et al. (2016, 2018) — Ecological Momentary Intervention methodology, which Stillform IS. Bandura (1977) self-efficacy and sense of mastery — mastery experience is the strongest source of self-efficacy. Brewer et al. (2011) trait-level change in experienced meditators. Lieberman 2007 amygdala reactivity reduction with repeated affect labeling. Lehrer et al. (2020) HRV biofeedback autonomic flexibility. Schultz (1998) on dopamine and reward prediction error + Wood & Rünger (2016) on habit formation — context-cue pairing wires practice in. Meichenbaum (1985) stress inoculation — practice when calm so it is available under pressure. Ecker, Ticic & Hulley (2012); Schiller et al. (2010, *Nature*); Lane et al. (2015) — memory reconsolidation. Features: streak tracking (context-cue pairing only, not gamification), daily practice loop, repeated Reframe sessions on recurring triggers, AI session-count graduation.
...
**Reconsolidation-window mismatch (M1) — applied, not new (June 23 2026).** Operationalizes the reconsolidation science above: when a user-CONFIRMED recurring pattern reactivates in a session, the app surfaces — suggestively, as a question, timed to the activated window — a likely mismatch computed by arithmetic from the user's own logged history (the historically-expected outcome did not follow a known trigger), for the user to agree with or correct. The felt contradiction delivered while the learning is active is the active ingredient (Ecker et al. 2012). Integrity: the app COMPUTES the candidate, never asserts the break; the user's agreement is the truth source; never a verdict. No new science introduced.
...
## Window of Tolerance (Supporting Framework for Body-First Noise-Cutting)

> **Note:** Earlier versions of this section claimed Window of Tolerance is "The Entire Product Purpose." That claim is INCORRECT per STILLFORM_FRAMING_LAW.md. Stillform's product purpose is the metacognition practice (concept-formation). The Window of Tolerance model is a useful supporting framework for understanding what the body-first noise-cutting tools (Breathe, Body Scan) accomplish — specifically, reducing physiological activation enough that the metacognitive work can land. Window-management is what the noise-cutters DO; it is not what Stillform IS.

**What it does:** Window of Tolerance is a clinical model that grounds why the body-first noise-cutting tools work. When physiological activation is too high (hyperarousal — panic, rage, overwhelm) or too low (hypoarousal — numbness, shutdown, disconnection), the user's cognitive bandwidth for the metacognitive work is reduced. Breath and body scan tools reduce activation enough that the concept-building work (chip selection, signal log, Reframe analysis) can land.

**The science:** The window of tolerance is the optimal arousal zone where a person can think clearly, regulate emotions, and engage in higher-order cognitive work — including the metacognitive analysis that Stillform's practice requires. Trauma, stress, and physical depletion narrow the window. Repeated practice with breath and body tools widens it over time (Lehrer et al. 2020 on HRV biofeedback autonomic flexibility gains).
...
- Porges (2011) — Polyvagal Theory — vagal nerve activation underlies window management
- Lehrer et al. (2020) — repeated HRV biofeedback practice widens the window over time

**Stillform application:** Breathe and Body Scan reduce activation enough for the metacognitive work to land. The bio-filter detects when physical state has already narrowed the window before the session starts (so the AI knows to weight body-first work more heavily). The pre/post rating tracks the user's self-reported movement toward the regulated zone — useful signal for the practice, NOT the goal of the practice. The goal is the concept-building that happens once the window is open enough for it.

**If they push back:** "There's a zone where the brain can do higher-order work — analysis, naming, reframing. Activation outside that zone makes that work impossible. Stillform's breath and body tools widen the zone so the metacognitive practice can land. The practice itself is the concept-building work that happens inside the zone."
...
**The devil's-advocate offer (considering the opposite).** An optional extension of the Thought Record's evidence step: alongside the user's own counter-evidence, the app can offer the single strongest case for the OTHER read of the situation — a counter-case the user weighs and then re-rates against. Actively considering the opposite is a well-established corrective; it reduces biased assimilation and overconfidence more than simply trying to "be fair," because it forces the mind to generate the evidence it was skipping. The user always stays the authority: the offer is take-it-or-leave-it, it never re-rates for them, and it challenges the BELIEF the user chose to test — never the person, never the feeling. HARD BOUND (safety, not optional): it declines to argue when the thought is a value, a boundary, a grief, or an accurate read of a genuinely bad situation. Those are not distortions to correct, and a counter-case there would invalidate rather than sharpen.
- Lord, Lepper & Preston (1984) — "considering the opposite" corrects biased social judgment better than demand-laden "be unbiased" instructions — JPSP 47(6):1231–1243
- Mussweiler, Strack & Pfeiffer (2000) — considering the opposite reduces the anchoring effect (compensates for selective accessibility) — PSPB 26:1142–1150
