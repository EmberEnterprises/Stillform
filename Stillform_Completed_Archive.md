...
- **6.4 checklist (parts a+b):** Science Sheet — **added** an After-Action Review entry (Tannenbaum & Cerasoli 2013 / Ellis & Davidi 2005 / Gollwitzer 1999). Privacy — no new data (post-event uses the same session record as any beat). Tutorial — N/A. FAQ — pending its screen. **6.4c (the manual entry surface) is an open fork for Arlin** — beats are time-routed or `?beat=`; post-event is the first manual beat, and canon (no parallel entry points; home = one pill) constrains where the affordance can live.
