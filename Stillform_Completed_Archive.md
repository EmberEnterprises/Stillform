...
**Phase 6.4 — Post-event reflection (5th beat), parts a+b (entry-agnostic):**
- **6.4a — Config + registration** (`b827269`). `POST_EVENT_CONFIG` in beatConfig.js — standard shape, Notice "What just happened?" (full-spectrum chips; an event leaves any state, wins included per framing law), reframe `includeTodayThread:false` (single episode, not a day), Close "Logged." carry-forward + Deep Regulate, NO daily completion flag (manual + repeatable — never advances the time-router). Registered in `CONFIGS`; `beat.js` `getBeatOverride` allowlist + JSDoc gained "post-event"; fixed a stale comment claiming Reset registers as a beat config. Testable via `?beat=post-event`.
- **6.4b — AI post-mortem addition** (`b827269`). `BEAT_ADDITIONS["post-event"]` in reframe.js — after-action review, three moves (what worked / what didn't [self-verdict catch] / what to keep [Gollwitzer if-then = the close takeaway]); single-episode bounded; operator voice; zero-fab. Auto-fires via the existing `hasOwnProperty` push; the eod todayThread block stays eod-gated.
- **6.4c — Manual entry** (`a935844`, **Arlin's call: the progress surface**). Entry on My Progress (developmental landing, Home footer "Progress" link) as a distinct ACTION above the diagnostic stack — "Break down something that just happened" — kept apart from the view-only mirrors. Launch: Spine `forcedBeat` one-shot (forcedBeat → `?beat=` → time-router); AppV2 `spineBeat` set to "post-event" on that target, cleared on exit + any normal Home launch (never sticky). Off-home per canon. **→ Phase 6.4 COMPLETE.**
- **6.4 checklist (parts a+b):** Science Sheet — **added** an After-Action Review entry (Tannenbaum & Cerasoli 2013 / Ellis & Davidi 2005 / Gollwitzer 1999). Privacy — no new data (post-event uses the same session record as any beat). Tutorial — N/A. FAQ — pending its screen. **6.4c resolved (Arlin: the progress surface)** — entry on My Progress; see the 6.4c bullet above.

**Phase 6.5 — Scripts (confidant-voice), built with defaults for on-screen review:**
- **Ground-truth:** the backend `netlify/functions/scripts.js` (303 lines, May 23) already existed — "State to Statement," input `{recipient, situation, outcome, channel}` → `{script, tone, note}` (+ crisis-redirect), voice already operator-tier/confidant. So 6.5 = build the client + ride-out, not a from-scratch feature; the "confidant-voice rewrite" was effectively satisfied by the existing prompt (verified, not rewritten).
- **6.5a — Client API** (`5623962`). `src/v2/lib/scriptsApi.js` — `generateScript(...)` → POST `/.netlify/functions/scripts`; models reframeApi; handles 403/429/non-ok + crisis-redirect + empty; no install_id (origin + IP-rate-limit gated).
- **6.5b — Scripts surface** (`50f266a`). `src/v2/screens/spine/Scripts.jsx` — self-contained: situation (seeded from the close takeaway, editable) + recipient + outcome + channel → generate → the line + move-label + note → Copy / Try another / Edit / Done; crisis + error + generating phases; ephemeral; not gamified; Plausible Script Generated / Copied.
- **6.5c — Ride-out at Close** (`50f266a`). A quiet "Need the words for it? ›" on the close next-move beat → `onWantScript(takeaway)` → Spine `scripts` step → Scripts; both exits route home. Quiet + optional — never blocks the close.
- **Built with DEFAULTS for Arlin to see live, then decide** (she works from the screen, not prose): ride-out placement, seed-from-session vs fresh, saved vs ephemeral, breathing-after-Scripts. Checklist: Plausible done; privacy = ephemeral (no new data); no new mechanism science (application-layer takeaway — the voice contract is the integrity layer, not a research claim); tutorial N/A; FAQ pending its screen. **→ Phase 6 fully built pending Arlin's review + deploy.**

**Phase 7 — Pre-event Brief v2 (built; defaults for on-screen review):**
- **Ground-truth:** `pre-event-brief.js` backend already existed (347 lines, May 23, 4-section gpt-4o artifact). Calendar entry = Phase 9, so Phase 7 = manual entry. Rehearsal was genuinely new (no backend).
- **7a — Client API** (`f3b9a28`). `preEventBriefApi.js` — `generatePreEventBrief(...)` → `{hardware, risks, moves, recovery}`; models scriptsApi.
- **7b — Brief surface** (`1a03bf1`). `src/v2/screens/PreEventBrief.jsx` (standalone screen, not a beat): event input → 4-section operator-tier brief → optional priming breath (box→quick-reset). Ephemeral.
- **7d — Manual entry** (`1a03bf1`). Second My Progress action "Prep for something coming up" paired with post-event's "Break down something that just happened"; AppV2 target → screen `pre-event-brief` (onDone→home, onExit→my-progress).
- **7c — Rehearsal** (`7cae25e`, **chose a new function**). NEW `netlify/functions/rehearsal.js` (scripts.js-modeled: CORS / 8-min IP rate-limit / crisis detection / gpt-4o json / voice-contract validation) → `{exchanges:[{they,you}] (2-3), note}` ("they" = realistic moments, never predictions; "you" = operator say-ready). `rehearsalApi.js` + integrated into the brief ("Rehearse it ›" → if-they-say/you pairs). NEW backend fn → deploys with the stack.
- **Checklist:** Science Sheet gained a Pre-event Brief (+ Rehearsal) entry (Barrett / Heider-Lazarus / Gollwitzer / Sheppes&Gross / Driskell 1994); Plausible (Pre-event Brief + Rehearsal Generated); ephemeral (no new data); tutorial N/A; FAQ pending. **→ Phase 7 built pending Arlin's review + deploy.**

**Phase 6 & 7 — Framing·Science·Flow parity pass (retroactive, May 28, at Arlin's request — these built today carried Science Sheet + checklist sweeps but not the inline F·S·F given to 8/10/EB; framing re-verified by grep on the actual files):**

*Framing note from the grep:* the only "regulate"/"calm"/"anxious" hits in Phase 6/7 are legitimate, NOT product positioning — (a) science citations in comments (Sheppes & Gross "anticipatory regulation" / "downregulation"), (b) the breathing-tool name "Deep Regulate" (an established option that serves the practice — framing law: regulation tools clear the channel), (c) the "anxious" feel-state chip (granularity vocabulary / required emotion coverage). No banned product framing.

- **PCE.1 — Structured close** (`f07d0ed`/`9f329fb`). *F·S·F:* Framing — metacognition→action, plasticity premise, user-authored takeaway (Pillar A). Science — Gollwitzer 1999 (implementation intentions) + Bandura (commitment). Flow — active close (takeaway → next-move → lock-in), persisted, surfaced to AI.
- **PCE.2 — Reconsolidation loop** (`6f06433`). *F·S·F:* Framing — reactivate-and-update a prior frame; plasticity is the premise; zero-fab, reflect-don't-score. Science — Ecker / Schiller / Lane (memory reconsolidation). Flow — gated on a real prior frame; no client change.
- **PCE.3 — Degradation layer / self-verdict catch** (`27a8681`). *F·S·F:* Framing — never validate "I've always / I'll never change / that's just who I am" (the framing law's plasticity premise); name it as the loop's output, redirect. Science — Wells MCT, Hitchcock 2024. Flow — always-on reframe.js block unifying distributed loop-handling.
- **6.2 — Move card** (`25fb5f2`/`384adfe`/`bd58663`). *F·S·F:* Framing — in-the-moment body-first disruptor that cuts noise so the practice can land (not regulation-as-product); dual exit, not gamified (CANON §10). Science — Porges / Levine / Sokolov / Lieberman / Kross / Wells (database.js + Science Sheet). Flow — chip→select (never null)→runner→dual-exit; body→cognitive handoff; no back-to-back repeat.
- **6.3 — Reset / urge-surf** (`baa7c96`/`9837908`). *F·S·F:* Framing — impulse-control practice; neutral exits, never scored (CANON §10); anti-compulsion (dopamine-awareness concern). Science — Marlatt urge-surfing, Bowen MBRP, Lieberman (Science Sheet). Flow — name→watch ~22s→act-or-don't; cross-route to Move.
- **6.4 — Post-event reflection / AAR** (`b827269`/`a935844`). *F·S·F:* Framing — after-action review (what worked / what didn't [self-verdict catch] / what to keep); full-spectrum chips (wins included per framing law); single-episode bounded; zero-fab. Science — Tannenbaum & Cerasoli 2013, Ellis & Davidi 2005, Gollwitzer (Science Sheet). Flow — 5th beat, manual + repeatable, never advances the time-router; My Progress entry.
- **6.5 — Scripts / State-to-Statement** (`5623962`/`50f266a`). *F·S·F:* Framing — operator/confidant voice turning a takeaway into a deployable line; application layer (applies the practice, no new mechanism claim); not gamified; ephemeral. Science — application-layer (affect-labeling / clarified language); no forced mechanism science (logged). Flow — seeded from takeaway → generate → copy/retry; crisis + error phases; ride-out at Close.
- **Phase 7 — Pre-event Brief + Rehearsal** (`f3b9a28`/`1a03bf1`/`7cae25e`). *F·S·F:* Framing — anticipatory prep + rehearsal; operator voice; "they" lines are possibilities to rehearse, never predictions-of-fact (zero-fab); recovery = downregulation cited as science, not positioned as the product. Science — Barrett 2017, Heider 1958/Lazarus 1991, Gollwitzer 1999, Sheppes & Gross 2011, Driskell 1994 (Science Sheet entry added). Flow — standalone screen; My Progress entry (paired with post-event); ephemeral; rehearsal sub-flow; native calendar entry = Phase 9.

**Phase 8 — Pricing surface (v2 client over the already-LIVE payments backend; built to its limit):**
- **8a — Status client** (`098fdfa`). `subscriptionApi.js` getSubscriptionStatus() → subscription-status?install_id → {isSubscribed}. Read-only plumbing. *F·S·F:* Framing — none (infra). Science — N/A. Flow — fail-OPEN documented (gating built on it must never lock out a payer on a blip).
- **8b — Value-framing paywall** (`df57bcd`). `Paywall.jsx`: WHAT STILLFORM IS before the ask → §291 Quick-Breathe-free stated → what's included → annual $9.99 (default) / monthly $14.99 → Start → startCheckout; ?paywall=1 review hook (no permanent entry until 8c); checkout helper + CHECKOUT_URLS slot (Arlin's LS links); Plausible Paywall Viewed + Checkout Started. *F·S·F:* Framing — PASS, audited line-by-line vs the banned list: zero regulate/calm/wellness, metacognition→expansion, composure as felt-outcome, §291 honored, purely positive. Science — rests on the established granularity/metacognition spine, no new claim. Flow — review-only until timing decided (correct), graceful "almost ready" (no dead link), telemetry in.
- **8c — Timing + gating** (`ee3cb39`). `gating.js` shouldGate() gates the session ONLY when confidently past FREE_SESSION_LIMIT (3) AND confirmed not-subscribed; any uncertainty → false (FAIL-OPEN). Knobs GATING_ENABLED + FREE_SESSION_LIMIT. Cache in subscriptionApi (error preserves prior status). AppV2 gates onBeginSession. *F·S·F:* Framing — §291 **verified in code**: Quick Breathe is a Home pill opening BreatheOverlay via local state, independent of the gated onBeginSession — the free valve is NOT behind the wall. Science — N/A (the 3-session timing serves continuity = the neuroplastic-repetition requirement). Flow — fail-open is the load-bearing safeguard; LIVE on next deploy (non-subscribers past 3 sessions incl. testers hit the wall — flip the knobs to defuse).
- **AUDIT FLAGS (honest — not framing fails):** (1) prices hardcoded $14.99/$9.99 while the May-16 $19.99-at-launch question is still deferred — revisit before launch. (2) Privacy/Terms must cover billing before a real purchase ships (with 8d). (3) FAQ billing/cancel entry pending the FAQ screen.

**Phase 10 — Onboarding front door (greenfield in v2; a new user previously landed on Home with no frame):**
- **10a — Intro frame** (`47fcb8f`). `onboarding.js` (isOnboarded/setOnboarded; reuses stillform_onboarded; storage-blocked → treat onboarded so no one's trapped) + `Onboarding.jsx`; AppV2 routes un-onboarded → intro; ?onboard=1 review hook. *F·S·F:* Framing — PASS (highest stakes; metacognition→expansion, composure felt-outcome, PURELY POSITIVE / no define-by-negation; mirrors the paywall frame). Science — granularity/precision spine. Flow — first-run gate, fail-safe storage handling.
- **10b — How-it-works** (`f346f37`). Refactored to a frame→how→in stepped flow; the loop in the user's terms (coarse label → precise read → one takeaway → compounds). *F·S·F:* Framing — PASS (coarse→precise stated positively). Science — emotional granularity (Barrett/Hoemann). Flow — Back nav, Begin→onComplete.
- **Plausible (audit gap closed, `af9d5be`):** the F·S·F sweep caught onboarding firing no telemetry — added Onboarding Started + Completed (the conversion funnel the redesign exists to move).
- *Doc-consistency note:* prior entries said "Tutorial N/A — no v2 onboarding system"; an onboarding front door now EXISTS, so that rationale is superseded going forward.

**Error boundary** (`1a4c6cf`). `ErrorBoundary.jsx` wraps AppV2; self-contained fallback (inline tokens only, no component deps so a tree fault can't kill the fallback) — calm on-voice message + Reload; Plausible "App Error". *F·S·F:* Framing — fallback on-voice, owns the error, honest ("history is saved"); protects §291 (no white-screen → free valve survives a crash). Science — N/A. Flow — strong integrity win (render crash → graceful recovery, not a blank screen).
