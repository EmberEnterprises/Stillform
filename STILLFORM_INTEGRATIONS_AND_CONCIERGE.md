...
3. **Concierge is a third pillar — design philosophy, never a surface.** Stillform stands on three pillars: (1) metacognition practice for self-mastery, (2) emotional honesty as cognitive material, (3) concierge layer that handles logistics so the user does the work. The concierge is NOT a tab, a screen, or a feature. It's a structural behavior distributed across every existing surface. **CORRECTED 2026-07-08 (Arlin): the concierge is PROACTIVE — it speaks up with real offers, BOTH anticipatory (Today's Brief / Pre-event Brief) AND in-the-moment (Step Out), but only when it genuinely has something grounded in the user's data, and its assertiveness ADAPTS to the user's state (bio-filter / State Check). The "silent/quiet" rule (§4 below) refers ONLY to silent DATA input (don't surface raw tracker numbers) — NOT to the concierge staying silent. A prior session mis-read "don't be naggy" as "stay silent"; Arlin reversed it. Preserved kernel: never naggy, never surveillance, always dismissible. See STILLFORM_CANON.md "CONCIERGE DIRECTION CORRECTED 2026-07-08."**
