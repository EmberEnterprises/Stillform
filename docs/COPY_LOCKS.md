> **AUDIT 2026-09-14 — RETIRED.** Every anchor below (splash tagline, tutorial pages, setup bridge, Pulse, Focus Check, the 'metacognitive composure practice' FAQ answer) belonged to the deleted v1 frontend; none exists in v2 (verified by grep, 0 matches — except the reframe.js soft-entry greeting, which is prompt text, not copy). The v2 form of a copy lock is the doctrine-sync guard (scripts/check-doctrine-sync.mjs: four sentinel sentences across four carriers) plus the chip lockstep guard (scripts/check-chip-definitions.mjs). Kept for history; do not reconcile.
